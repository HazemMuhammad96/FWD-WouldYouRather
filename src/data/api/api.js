...
export function _getUsers() {
    return new Promise((res, rej) => {
        ...
        setTimeout(() => res({ ...tempUsers }), 1000)
    })
}

export function LogUser({ id, password }) {
...
export function getQuestionById(id) {
    return new Promise((res, rej) => {
        ...
        if (question)
            setTimeout(() => res({ ...question, author: users[question.author] }), 1000)
        else 
            setTimeout(() => rej('Question not found'), 1000)
    })
}
...
export function _updateQuestion(payload) {
    return new Promise((res, rej) => {
        ...
        if (!currentQuestion["optionOne"].votes.includes(payload.user) &&
            ...
            _saveQuestionAnswer({
                authedUser: payload.user.id,
                qid: payload.id,
                answer: payload.option
            }).then(() => res(payload));

        }
        else rej('You have already voted on this question')
